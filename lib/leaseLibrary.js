 ...
 * @returns {[Object]} An array of one or two dateRanges objects. If empty, error. If it contains a null value, the dateRange is fully reservated and is to be removed completely from DB.
 */
...
const dateRangeSplitter = (dateRangeToSplit, dateRangeToWithdraw) => {
  ...
  // TODO : double check if dateRanges overlap

  if (!dateRangeOverlaps(dateRangeToSplit, dateRangeToWithdraw)) return [];

  // TODO : checkif startDate is not superior to endDate. In that case, return empty array
  if (
  ...
  // TODO : check if malformed parameters. In that case, return empty array
  if (
  ...
  if (
    ...
    // TODO handle case where dateRangeToWithdraw starts before and ends within dateRangeToSplit
    if (
      new Date(dateRangeToWithdraw.startDate).getTime() <
        new Date(dateRangeToSplit.startDate).getTime() &&
      new Date(dateRangeToWithdraw.endDate).getTime() <=
    ...
    // TODO handle case where dateRangeToWithdraw starts within dateRangeToSplit and ends after it
    else if (
      new Date(dateRangeToWithdraw.startDate).getTime() >=
        new Date(dateRangeToSplit.startDate).getTime() &&
      new Date(dateRangeToWithdraw.startDate).getTime() <=
    ...
    // TODO handle case where dateRangeToWithdraw starts before and ends after dateRangeToSplit
    else if (
      new Date(dateRangeToWithdraw.startDate).getTime() <
        new Date(dateRangeToSplit.startDate).getTime() &&
      new Date(dateRangeToWithdraw.endDate).getTime() >
