 ...
 * @function
 * @memberof module:lib/leaseLibrary
 * @desc Returns true if a dateRange overlaps with at least one other in a dateRange array.
 * @desc Else returns false
...
const dateRangeSplitter = (dateRangeToSplit, dateRangeToWithdraw) => {
  ...
  // dateRangeToWithdraw cannot be as long or longer than dateRangeToSplit
  if (compareDateRanges(dateRangeToWithdraw, dateRangeToSplit) >= 0) {
    return null;
  }
  ...
  // cas 2: dateRangeToWithdraw termine en même temps que dateRangeToSplit => [{dr1}]
  else if (
  ...
  // cas 3 : dateRangeToWithdraw is in the middle of dateRangeToSplit => [{dr1},{dr2}]
  else {
    console.log("cas 3");

    let newStartRange1 = {
      startDate: dateRangeToSplit.startDate,
      endDate: new Date(
        new Date(dateRangeToWithdraw.startDate).getTime() - DAY_IN_MS
      ),
    };
